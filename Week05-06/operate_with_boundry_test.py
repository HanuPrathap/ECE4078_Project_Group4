...
class Operate:
    ...
    def boundary_check(self):
        """Warn or hit if the robot's 100 mm buffer touches/approaches the wall."""
        # Pose from EKF (shape (3,) or (3,1) -> cast to float)
        x = float(self.ekf.robot.state[0])
        y = float(self.ekf.robot.state[1])

        xmin, ymin, xmax, ymax = self.bound

        # Distance from the ROBOT CENTER to each edge (≥0 if inside)
        d_left   = x - xmin
        d_right  = xmax - x
        d_bottom = y - ymin
        d_top    = ymax - y
        dmin_center = min(d_left, d_right, d_bottom, d_top)

        # Clearance from the robot's BUFFER (100 mm radius) to the wall
        clearance = dmin_center - self.robot_radius_m  # <0 means penetration

        # Distance gauge (keep in UI): show both clearance and center distance
        gauge = f"clearance {max(clearance,0):.3f} m (center {dmin_center:.3f} m)"

        if clearance <= 0.0:
            # Buffer touches or crosses the wall
            self.notification = f"WALL HIT — {gauge}"
        elif clearance <= self.boundary_warn_m:
            # Near wall but not touching
            self.notification = f"WARNING: near wall — {gauge}"
        # else: leave any existing notification alone
...
if __name__ == "__main__":
    ...
    while start:
        ...
        operate.boundary_check()    # ← move to here so message isn’t overwritten
