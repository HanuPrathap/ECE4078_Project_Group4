...
RES        = 0.01     # m / cell
ROBOT_R    = 0.09     # robot radius (m)
MARGIN     = 0.06     # extra safety (m)
STANDOFF_R = 0.20     # m from fruit
HOLD_SECS  = 5.0      # s pause at standoff
...
def execute_motion_with_localization(ppi, localizer, motion_command, duration, ip="192.168.50.1"):
    ...
    while (time.time() - start) < duration:
        ...
        last = now
        # camera stream on robot is mirrored; your original code flips rv when not localhost
        drive_meas = measure.Drive(lv, -rv if ip != "localhost" else rv, dt)
        img = ppi.get_image()
        try:
            localizer.step(img, drive_meas)
...
def drive_to_point_with_localization(ppi, waypoint, robot_pose, localizer,
                                     ...
                                     ip="192.168.50.1", scale=None, baseline=None):
    ...
    # Read calibration once in main and pass in; fallback kept for compatibility
    if scale is None or baseline is None:
        try:
            ...
            raise RuntimeError(f"Calibration params not provided and couldn't be loaded: {e}")
    ...
    wheel = 40  # PWM ticks used in set_velocity
    ...
    if turn_t > 0.05:
        execute_motion_with_localization(ppi, localizer, [0, turn_dir], turn_t, ip)

    if drive_t > 0.05:
        execute_motion_with_localization(ppi, localizer, [1, 0], drive_t, ip)
...
def follow_path_with_localization(ppi, localizer, path_xy, skip=3,
                                  stop_within=None, hold_secs=0.0,
                                  ip="192.168.50.1", scale=None, baseline=None):
    """Follow a polyline with optional downsampling; on last point apply stop/hold."""
    ...
    for i, wp in enumerate(sampled):
        ...
        pose = drive_to_point_with_localization(
            ...
            ip=ip, scale=scale, baseline=baseline
        )
...
if __name__ == "__main__":
    ...
    # Targets (optional) to show and lightly inflate as soft obstacles
    target_points_xy = np.array(targets_xy, dtype=np.float64) if len(targets_xy) > 0 else None

    # ---- Build planning grid (once) ----
    costmap, occ, meta = build_costmap_fixed_squares(
    size_m=args.arena_size,
    aruco_points_m=aruco_true_pos,
    fruit_points_m=target_points_xy,
    res=args.res,
    robot_radius_m=args.robot_r,
    safety_margin_m=args.margin,
    aruco_size_m=0.09,   # e.g., 8 cm marker
    fruit_size_m=0.07,   # e.g., 6 cm fruit cube    
    )
    ...
    if args.show_map:
        visualize_costmap_detailed(
            ...
            np.array(targets_xy, dtype=np.float64) if len(targets_xy) > 0 else None,
            title="A* costmap"
        )
    ...
    if args.dry_run:
        ...
        for i, fruit_xy in enumerate(targets_xy, start=1):
            ...
            fruit_all = []
            if len(distractors_xy) > 0: fruit_all.extend(distractors_xy)
            if len(targets_xy) > 0:     fruit_all.extend(targets_xy)

            visualize_plan_with_robot_footprint(
                costmap, occ, meta, path,
                start_xy=current_xy, goal_xy=standoff_xy,
                robot_radius_m=args.robot_r,
                aruco_points=aruco_true_pos,
                fruit_points=np.array(fruit_all, dtype=np.float64),
                aruco_size_m=0.08,    # tweak to your marker size
                fruit_size_m=0.06,    # tweak to your cube size
                sample_step_m=0.02,   # smaller = denser footprint sampling
    ...
    try:
        for i, fruit_xy in enumerate(targets_xy, start=1):
            ...
            # Smooth for visualization (optional)
            path = smooth_polyline(raw_path, lam=args.smooth_lam, iters=args.smooth_iters)

            # choose which polyline to validate/drive
            poly_to_follow = path    # or 'path' if you want the smoothed one

            # if you have a soft band, set a threshold to keep away from it (e.g., <= 10)
            # if you only have hard obstacles, leave at 254
            safe_poly = make_collision_checked_path(
                costmap, meta, poly_to_follow,
                max_allowed_cost=254,      # or e.g. 10 to avoid soft band cells
                sample_step_cells=0.5      # sample every 0.5*res along segments
            )

            if safe_poly is None:
                print("  [Path refinement failed] Replanning from current pose…")
                # optional: re-plan here (e.g., lower skip, change keep-out, etc.)
                continue

            # You can still downsample a bit for fewer stops (after it's safe)
            safe_sampled = safe_poly[::max(1, int(args.skip))]
            if safe_sampled[-1] != safe_poly[-1]:
                safe_sampled.append(safe_poly[-1])

            follow_path_with_localization(
                ppi, localizer, safe_sampled,
                skip=1,   # already sampled; keep 1 to use the points we just built
            ...
            if args.show_each_leg:
                visualize_plan_over_costmap(costmap, occ, meta, path, current_xy, standoff_xy,
                                            title=f"Leg {i} (smoothed)")

            # Follow the (raw) path with waypoint skipping for fewer stops
            follow_path_with_localization(
                ppi, localizer, raw_path, skip=args.skip,
                stop_within=args.standoff, hold_secs=args.hold_secs,
                ip=args.ip, scale=scale, baseline=baseline
            )
...
# dry 
# python test.py --dry_run --show_map --show_each_leg

# live
# python test.py --ip 192.168.50.1 --show_map