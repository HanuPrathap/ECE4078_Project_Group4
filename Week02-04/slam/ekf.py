...
class EKF:
    # Implementation of an EKF for SLAM
    # state has all the positionn ofhte robot (first three) everythign else is landmarks positions
    # The state is ordered as [x; y; theta; l1x; l1y; ...; lnx; lny]


    # has methods for state, derivitive drivec, covariance drive, measure and derivitve measure - all part of robot
    # markers is a np.array of shape (2,N) n = no of landmarsk 
    # tag list is the tags for each landmark 
    # P is covariace matric for the state 
    # init_lm_cov = initial covarince for the landmark for new landmarsk 
    # robot_inti_stae is the intial sttae of the robot - sueful for protytyping aybe 
    # lm_pics is list of pygame images for landmark visualisation 
    # pilbot-pic = pygame image for robot visualisaion 
    ...
    def __init__(self, robot):
        ...
        self.P = np.zeros((3,3))
        self.init_lm_cov = 1e3 # this is set to very unsure 
    ...
    def reset(self): # funcutionn resets the state
        self.robot.state = np.zeros((3, 1)) # empty vector to store robot positon 
        self.markers = np.zeros((2,0)) # empty colum vector
        self.taglist = [] # empty tag list 

        # Covariance matrix
        self.P = np.zeros((3,3)) # set covarinec to 0
        self.init_lm_cov = 1e3 # set lm covariuance to high  - i think
        self.robot_init_state = None

    def number_landmarks(self): # returns how many coulnms are in markers which holds all entried of landmars 
        return int(self.markers.shape[1])

    def get_state_vector(self): # returns the state off the robot + all landmarks 
        state = np.concatenate(
            (self.robot.state, np.reshape(self.markers, (-1,1), order='F')), axis=0)
        return state
    
    def set_state_vector(self, state): # setting the state of everything
        self.robot.state = state[0:3,:]
        self.markers = np.reshape(state[3:,:], (2,-1), order='F')
    
    def save_map(self, fname="slam_map.txt"): # saves the current mpa, landmark positon , tag valuesm and their covariance 
        if self.number_landmarks() > 0:
            utils = MappingUtils(self.markers, self.P[3:,3:], self.taglist)
            utils.save(fname)

    def recover_from_pause(self, measurements): # research  more 
        if not measurements:
            return False
        else:
            lm_new = np.zeros((2,0))
    ...
    def update(self, measurements, raw_drive_meas): # added the raw_drive_meas
        if not measurements:
            return
        ...
        # get P prediction 
        P = self.predict(self, raw_drive_meas)
        Q  = self.predict_covariance(self, raw_drive_meas)

        innovation_matrix = (H * P * H.T + R)
    ...
    def state_transition(self, raw_drive_meas): # constructs and returns the state transition matric F(A) for the current state size using curretn state size and robots drive covariance 
